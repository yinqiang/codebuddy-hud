...
function resolveColorCode(colorName) {
    // Named preset
    if (ANSI_CODES[colorName])
        return ANSI_CODES[colorName];
...
// ─── Visual progress bar ────────────────────────────────────────────
export function coloredBar(percent, width = 10, options) {
    const { color = 'green', warningColor = 'yellow', criticalColor = 'red', warningThreshold = 70, criticalThreshold = 85, } = options ?? {};
