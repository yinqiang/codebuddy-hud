 ...
 */
import { getModelName, getProjectName, getCostUsd, getLinesAdded, getLinesRemoved, getSessionId, getVersion, } from '../stdin.js';
import { model as modelColor, project as projectColor, git as gitColor, gitBranch as gitBranchColor, cost as costColor, label, dim, } from './colors.js';
...
export function formatDuration(ms, lang = 'en') {
    ...
    if (mins < 1)
        return s.lessThanMinute;
    if (mins < 60)
        return `${mins}${s.minute}`;
 ...
 */
function renderCompactSessionLine(ctx) {
    ...
    if (gitConfig.enabled && gitStatus) {
        ...
        if (gitConfig.showAheadBehind) {
            if (gitStatus.ahead > 0)
                gitParts.push(` ${s.gitAhead}${gitStatus.ahead}`);
            if (gitStatus.behind > 0)
                gitParts.push(` ${s.gitBehind}${gitStatus.behind}`);
        }
        if (gitConfig.showFileStats && gitStatus.fileStats) {
            const { modified, added, deleted, untracked } = gitStatus.fileStats;
            const statParts = [];
            if (modified > 0)
                statParts.push(`${s.modified}${modified}`);
            if (added > 0)
                statParts.push(`${s.added}${added}`);
            if (deleted > 0)
                statParts.push(`${s.deleted}${deleted}`);
            if (untracked > 0)
                statParts.push(`${s.untracked}${untracked}`);
    ...
    if (display.showCodeStats) {
        ...
        if (added > 0 || removed > 0) {
            const statsParts = [];
            if (added > 0)
                statsParts.push(`+${formatLines(added)}`);
            if (removed > 0)
                statsParts.push(`-${formatLines(removed)}`);
 ...
 */
function renderExpandedSessionLine(ctx) {
    const { stdin, gitStatus, sessionDuration, config } = ctx;
    const display = config.display;
    const colors = config.colors;
    const s = getStrings(config.language);
    ...
    if (gitConfig.enabled && gitStatus) {
        ...
        const gitStr = gitParts.join(' ');
        lines.push(dim(`${s.git}: `) + gitBranchColor(gitStr, colors));
    }
    // ── Duration (line 3) ─────────────────────────────────────────
    if (display.showDuration && sessionDuration) {
        lines.push(dim(`${s.duration} `) + label(sessionDuration, colors));
    }
    // ── Cost (line 4) ─────────────────────────────────────────────
    if (display.showCost) {
        const costUsd = getCostUsd(stdin);
        if (costUsd !== null) {
            lines.push(dim(`${s.cost} `) + costColor(formatCost(costUsd), colors));
        }
    }
    // ── Code stats (line 5) ───────────────────────────────────────
    if (display.showCodeStats) {
