...
async function computeGitStatus(cwd, options) {
    ...
    try {
        ...
        if (showFileStats && dirtyResult && 'fileStats' in dirtyResult && dirtyResult.isDirty) {
            try {
                const { stdout: numstatOut } = await execFileAsync('git', ['diff', '--numstat', 'HEAD'], { cwd, timeout: GIT_TIMEOUT, encoding: 'utf8' });
 ...
 */
function parseNumstat(numstatOutput, fileStats) {
