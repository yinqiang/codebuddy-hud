...
export async function main() {
    ...
    try {
        ...
        if (profile) {
            process.stderr.write(`[hud-profile] stdin:${(t1 - t0).toFixed(0)}ms config:${(t2 - t1).toFixed(0)}ms git:${(t3 - t2).toFixed(0)}ms transcript:${(t4 - t3).toFixed(0)}ms render:${(t5 - t4).toFixed(0)}ms total:${(t5 - t0).toFixed(0)}ms\n`);
        }
    }
    catch (error) {
